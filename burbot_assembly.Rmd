...
ssh -i ~/.ssh/mandykey.pem -l ubuntu ec2-54-203-158-28.us-west-2.compute.amazonaws.com
...
# Script to run rcorrector on all the sequence files: 
```{bash}

for infile in *R1_001.fastq.gz
do
    base=$(basename $infile R1_001.fastq.gz)
    perl ~/rcorrector/run_rcorrector.pl -1 ${base}R1_001.fastq.gz -2 ${base}R2_001.fastq.gz -k 32 -t 30
done
...
# t=30 is 30 threads to parallelize to decrease running time. Erin's took 20 hours to run 
...
## Script to concatenate files together 
```{bash}

mkdir concat
for file_start in burbot-Ma03*R1* burbot-Mb05*R1*
do  
  ...
  cat $allR1 > ./concat/${outfile}.fastq.gz
done
...
```{bash}
echo "80ab57f0c1ea7d90dc1d862c47c30e8d  burbot-Ma01-C_S203_L004_R1_001.fastq.gz" | cut -f1 -d" "   
echo "MD5 (burbot-Ma01-C_S203_L004_R1_001.fastq.gz) = 80ab57f0c1ea7d90dc1d862c47c30e8d" | cut -f2 -d"=" | cut -f2 -d" "

slims= grep "^.+\.\.${fish1}" md5.txt

slims= grep '^.*burbot-Ma01-C_S203_L004_R1_001.fastq.gz' md5.txt
grep '^.*\.\.'burbot-Ma01-C_S203_L004_R1_001.fastq.gz md5.txt
